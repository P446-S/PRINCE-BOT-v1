...
let settings = {
  prefix: ".",
  ownerNumber: ["234xxxxxxxxxx@s.whatsapp.net"], // ← CHANGE TO YOUR NUMBER
  ownerName: "Chinenye",
  badwords: [],
  blockedCountryCodes: [],
  alwaysonline: false,
  ...
  run: async ({ sock, msg, body, cmd, isOwner }) => {
    ...
    try {
      ...
      if (cmd === "delanticallmsg") {
        ...
        saveSettings()
        return sock.sendMessage(from, { text: "✅ Anti-call message reset to default" }, { quoted: msg })
      }
      ...
      if (cmd === "setownernumber") {
        if (!text) return sock.sendMessage(from, { text: "❌ Example: .setownernumber 234xxxxxxxxxx" }, { quoted: msg })
        settings.ownerNumber = [`${text}@s.whatsapp.net`]
        saveSettings()
        return sock.sendMessage(from, { text: `✅ Owner number set to: ${text}` }, { quoted: msg })
      }
      ...
      if (cmd === "setmenuimage") {
        ...
        saveSettings()
        return sock.sendMessage(from, { text: `✅ Menu image set to:\n${text}` }, { quoted: msg })
      }
